...
#------------------------------------------------------------------------------
# Installs Surveillance Station 9.2.0-11289
# The last version of Surveillance Station that had software motion detection
#
# Installs Advanced Media Extensions 3.1.0-3005
# The last version Advanced Media Extensions which had HEVC
#------------------------------------------------------------------------------
# TODO
...
# Get installed SurveillanceStation version
ss_version=$(/usr/syno/bin/synopkg version SurveillanceStation)
if check_pkg_installed SurveillanceStation && [[ ${ss_version:0:2} != "90" ]]; then
    # Uninstall SurveillanceStation (wrong version)
    echo ""
    package_uninstall SurveillanceStation "SurveillanceStation"
fi
...
if ! check_pkg_installed SurveillanceStation && [[ $ss_version != "90.2.0-11289" ]]; then
    ...
    # Prevent package updating and "update available" messages
    echo "Preventing Surveillance Station from auto updating"
    /usr/syno/bin/synosetkeyvalue /var/packages/SurveillanceStation/INFO version "90.2.0-11289"
    package_start SurveillanceStation "Surveillance Station"
    rm -f "/tmp/SurveillanceStation-${cputype}-9.2.0-11289.spk"
else
    echo -e "\n${Cyan}Surveillance Station${Off} $ss_version already installed"
fi
...
if check_pkg_installed SurveillanceStation; then
    if ! package_is_running SurveillanceStation; then
        package_start SurveillanceStation "Surveillance Station"
    fi
fi
